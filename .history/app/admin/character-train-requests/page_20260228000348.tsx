...
import { completeTrainRequest, rejectTrainRequest } from '@/app/actions/character/train';
...
export default function CharacterTrainRequestsPage() {
    ...
    // Listen to training requests in real-time
    useEffect(() => {
        const db = getFirestore(app);
        const requestsRef = collection(db, 'character-train-requests');
        const q = query(requestsRef, orderBy('requestedAt', 'desc'));

        const unsubscribe = onSnapshot(
            q,
            (snapshot) => {
                const requestsData: TrainRequest[] = [];
