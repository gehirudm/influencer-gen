...
export async function submitTrainRequest(characterId: string) {
    try {
        ...
        await createNotification({
            ...
            message: `We've received your training request for "${characterData.name}". Your character will be ready in approximately 2–6 hours. We'll notify you as soon as it's available to use!`,
            link: '/character',
        });
...
/**
 * Admin action: Fetch all training requests. Uses firebase-admin to bypass client security rules.
 */
export async function fetchTrainRequests() {
    try {
        const uid = await getSessionUid();
        if (!uid) {
            return { success: false, error: 'Authentication required', requests: [] };
        }
        ...
        if (!systemData?.isAdmin) {
            return { success: false, error: 'Admin access required', requests: [] };
        }
        ...
        return { success: false, error: error.message || 'Failed to fetch training requests', requests: [] };
    }
}
